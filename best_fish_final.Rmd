...
```{r}
ggplot(data = demog_best[-301,], aes(x = site, fill = sex)) + geom_bar(position = "fill") + theme_classic() + scale_fill_brewer(palette = "Set1") + ylab("Percent") + xlab("")
```
```{r}
...
```{r}
lf_density_by_time <- ggplot(data = lionfish_best, aes(x = survey, y=lf_quad, fill = survey)) + geom_bar(stat = "summary", fun.y = "mean") + labs(y = "Mean Density (no/250m^2)") + stat_summary(geom = "errorbar", fun.data = mean_se) + scale_x_discrete(name = "Survey Number", limits=c("1", "2", "3", "4", "5", "6")) + theme_classic() + geom_line(aes(x = survey, y = lf_removed, color = site)) 
